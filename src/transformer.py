...
def generate(embeddings, vocabulary, WK, WO, WQ, WV):
    ...
    for embedding in embeddings:
        ...
        provided_word_idx = np.argmax(XK, axis = 1) 
        provided_words = [vocabulary[idx] for idx in provided_word_idx]
        
        predicted_word_idx = np.argmax(O, axis = 1)
        predicted_word = [vocabulary[idx] for idx in predicted_word_idx]
        
        generated_sentence = provided_words + predicted_word
...
def cross_entropy_loss(O, Y):
    ...
    '''
    L = 0
    for hyp, ref in zip(O, Y):
        L -= np.log(max(hyp[np.argmax(ref)], sys.float_info.min))
    dO = -Y/O
    return L, dO
...
def gradient(XK, XQ, Y, WK, WO, WQ, WV, A, C, K, O, Q, V):
    ...
    dO_softmax =  O * (dO - np.sum(O * dO, axis=1, keepdims=True))
    dWO = C.T @ dO_softmax
    ...
    dA = dC @ V.T
    dQKT = A * (dA - np.sum(A * dA, axis=1, keepdims=True))
